...
- 放大页支持 RIFE 插帧（rife-v4.6 模型，ncnn Vulkan 推理）：2x / 3x / 4x 导出时在增强后的
  帧间生成中间帧，输出帧率按倍数提升。需要程序目录带 `rife_host.dll` 与 `models/rife-v4.6`；
  插帧在增强后进行，VSR / DLSS 只处理源帧，代价更小。
